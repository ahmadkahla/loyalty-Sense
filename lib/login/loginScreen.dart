...
//
// // ============================================================================
// // 🎨 COLOR HELPERS (للدعم Dark/Light)
// // ============================================================================
//
// class _AppColors {
//   // ============================================================
...
//
//   // ============================================================
//   // 🎯 VERIFY OTP — التحقق يحدث أثناء الدوران
//   // ============================================================
//
...
//
//     // ============================================================
//     // 🎯 1. ابدأ حالة "جاري التحقق" + الدوران
//     // ============================================================
//     setState(() {
//       _isVerifying = true;
//       _otpVerified = false;
...
//
//     // ============================================================
//     // 🎯 2. التحقق + الحد الأدنى للدوران — بالتوازي
//     // ============================================================
...
//           });
//           _showMessage(
//             message: 'لا يمكن العثور على بيانات العضوية',
//             icon: Icons.error_outline_rounded,
//           );
//           return;
//         }
//
//         await prefs.setString('user_name', customerName);
...
//               const SizedBox(height: 19),
//
//               _buildOtpBoxes(isSmall, otpBoxWidth, otpBoxHeight, isDark),
//
...
//                       return _buildSingleOtpBox(
//                         index: index,
//                         otpBoxWidth: otpBoxWidth,
...
//
//   // ============================================================
//   // 🎯 المربعات في مسار دائري
//   // ============================================================
//   Widget _buildCircularOtpBoxes({
//     required bool isSmall,
//     required double otpBoxWidth,
...
//               index: index,
//               otpBoxWidth: adjustedBoxSize,
//               otpBoxHeight: adjustedBoxSize,
//               isSmall: isSmall,
...
//     required bool isDark,
//   }) {
//     // 🎨 في الوضع الداكن: gradient بنفسجي-وردي
//     return SizedBox(
...
//
//         // ==========================================================
//         // 🆕 مستخدم جديد (customerNo فاضي) → CompleteProfileScreen
//         // ==========================================================
//         if (customerNo.isEmpty) {
//           debugPrint(
...
//
//         // ==========================================================
//         // ✅ مستخدم موجود → احفظ وروح HomeScreen
//         // ==========================================================
...
//
//         // ==========================================================
//         // 🆕 تحديد إذا الخطأ يعني "المستخدم غير موجود"
//         // ==========================================================
//         final isNotFound =
//             statusCode == 404 ||
  ...
  Future<void> _verifyOtp() async {
    ...
    await result.fold(
      (userData) async {
        ...
        // ==========================================================
        // ✅ مستخدم موجود → احفظ بيانات الـ API فقط
        // ==========================================================
        debugPrint('✅ المستخدم موجود → HomeScreen');

        // 🧹 1. امسح كل بيانات المستخدم السابق
        await prefs.remove('user_name');
        await prefs.remove('user_email');
        await prefs.remove('user_birth_date');
        await prefs.remove('user_customer_no');
        ...
        // 💾 2. احفظ البيانات الجديدة (اللي رجعت من الـ API)
        await prefs.setString('user_name', customerName);
        await prefs.setString('user_customer_no', customerNo);
        await prefs.setString('user_phone', phone);
        await prefs.setBool('logged_in', true);

        // 📧 3. الإيميل — فقط إذا موجود من الـ API
        ...
        // 🎂 4. تاريخ الميلاد — فقط إذا موجود من الـ API
        ...
        if (apiBirthday.isNotEmpty) {
          try {
            ...
            debugPrint('🔴 [getUserByPhone] birthday parse error: $e');
          }
        }
